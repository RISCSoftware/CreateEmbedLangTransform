...
The tool takes 2 or 3 command line arguments:

```bat
CreateEmbedLangTransform.exe <target-and-reference-msi> <source-msi> [<language-identifier>]
```

If the language ID (the third argument) is omitted, it is read from the summary information of the source MSI.
...
```bat
CreateEmbedLangTransform.exe Setup.msi Setup_de-de.msi
```

The language ID (in this case "1031") can optionally be passed on the command line for slightly faster processing.
...
CreateEmbedLangTransform.exe $(TargetFileName) de-de\$(TargetFileName)
...
1. It embeds the transform as a substorage under the name equal to the language ID (the third argument if given) (equivalent to the `WiSubStg.vbs` script).
1. To support WiX v.4 or later, the tool also adds the language ID to the Summary information property "Template" if it isn't already, so that the MSI package is recognized as multilingual by Windows Installer (equivalent to the `WiLangId.vbs` script).
